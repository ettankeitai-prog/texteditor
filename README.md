...
- 終了・再読み込み前に未保存タブとWorkspaceの保存完了を待つハンドシェイクを追加
- 自動保存失敗時の限定再試行と、Workspaceインポート中・完了後の書き込みロックを追加
- ENOENT以外のJSON読み込みエラーを起動失敗として通知し、破損JSONを既定値で上書きしない
- 同一保存先への書き込みとRemote Inboxの同時更新を直列化
- Remote Webに未保存変更の保護と処理中の多重操作防止を追加
- 左ペインを「Open Tabs / 開いているタブ」とし、全体検索・新規タブを画面内から実行可能に変更
- 全タブを閉じた状態に、新規タブとRecent / Closedへの導線を追加
- 単一ペインのタイトル・小タブ行を統合し、分割時だけ各ペインのコンパクトなヘッダーを表示
- 保存状態をステータスバーへ一本化し、内部タブIDとデータ保存パスの常時表示を廃止
- グループごとに「開いている数 / 全体数」を表示し、狭い画面ではミニマップを自動的に非表示
...
- Same-target mutations are serialized; stale revisions continue to return `409 Conflict`
- Legacy data with duplicate Remote Inbox target titles is treated as ambiguous and is never written automatically
- Unsaved Remote Web edits are protected before reload or target changes, and mutation controls are disabled while a request is running
...
Remote Inbox のブラウザー画面テストも含むため、初回のみ Chromium を準備します（依存関係の追加ではなく、Playwright 用ブラウザーの取得です）。
...
- 終了直前の保存完了ハンドシェイクと破損JSONの非上書き
- サイドバーの検索・新規作成、全タブを閉じた際の空状態導線
- 単一／分割ヘッダー、内部情報の非表示、狭幅時のミニマップ非表示
- グループの開いている数／全体数、新規タブのタイトル全選択と Enter 後の本文フォーカス
- Import後の編集ロックと再起動導線
- 同一受信先へのRemote Inbox同時更新とRemote Webの未保存保護
...
JSONファイルが存在しない場合は従来どおり既定値を作成します。一方、構文破損、権限不足、読み込み失敗など `ENOENT` 以外のエラーは起動失敗として画面へ通知し、元のJSONを空の既定データで上書きしません。

## 現在の仕様

- Electron アプリとして起動
- 同じuserDataを使う多重起動を防止し、2回目の起動要求では既存ウィンドウを前面へ表示
- OS 標準の Electron メニューバーから主要操作を実行
- アプリ画面内の独自 File / Edit / View メニューは非表示
- 左ペインに現在開いているタブを表示
- 左ペイン見出しの検索ボタンと新規作成ボタンから頻用操作を実行可能
- 全タブを閉じた場合は、新規タブと Recent / Closed を空状態に表示
- 左ペインのタブを大項目グループで整理可能
- 各大項目に「開いている数 / 全体数」を表示
- 左ペインの幅をドラッグで変更可能
...
- 新規タブ作成後はタイトルを全選択し、Enterで確定すると本文へフォーカス
- 単一ペインではタイトルと小タブを1行の共通ヘッダーに表示し、分割時だけ左右ペインのコンパクトなヘッダーを表示
...
- 本文データには行番号を保存しない
- 編集内容を自動保存
- 終了・Reload要求時は保留中の自動保存とWorkspace保存の完了後に処理を続行
- 保存失敗時は回数を制限して再試行し、失敗状態をステータスバーへ表示
- タブ単位のバックアップを自動作成
- アクティブタブ本文の全文コピー
- 起動時に以下を復元
  - アクティブタブ
...
- ダークモード、ライトモード切り替え
- UI 言語を英語 / 日本語で切り替え
- 右側に簡易ミニマップを表示し、狭いウィンドウや狭幅の全体検索中は自動的に非表示
- 内部タブIDとデータ保存パスは画面へ常時表示しない
...
- 既定受信先は候補一覧に必ず含まれ、旧設定で一覧から欠けている場合も自動的に補完します
...
同じ受信先への追記、全文保存、クリアは直列に処理します。同じrevisionによる同時更新は1件だけを保存し、後続の古い更新は `409 Conflict` とします。Remote Webで本文を変更した後、再読み込み、受信先変更、タブ閲覧への移動を行う場合は未保存変更を確認し、キャンセル時は入力内容を保持します。通信中は関連ボタンを無効化して二重送信を防止します。
...
- タブを閉じる操作はデータ変更ではないため、Remote Inboxタブも右クリックメニューから閉じられます
...
- 単一ペイン時は共通ヘッダー、分割中は左右ペインそれぞれのコンパクトなタブ名 input から名称を編集
- 左右ペインそれぞれで別の小タブを開ける
...
- タブを閉じても所属大項目は維持
- 大項目名の右側には、現在開いているタブ数と所属タブ総数を `1 / 4` の形式で表示
...
- Import 完了後は旧Workspaceを表示中のrendererを編集不可にし、再起動専用ダイアログを表示
- ダイアログの `Restart now` / `今すぐ再起動` から、新しく読み込んだWorkspaceを安全に開き直せる
...
- 復元確認中の Escape は `Cancel` / `キャンセル` と同じく正式な終了処理を行い、起動待機状態を残さない
...
rendererのビルド先は `dist/renderer/` だけを対象に毎回空にしてから生成するため、過去のハッシュ付きJS/CSSを配布物へ残しません。main、preload、sharedの出力先はrendererのクリーン対象外です。
